...
def user_playlists(username):
    ...
    try:
        ...
        From mediaserver.mediaCollection MC JOIN mediaserver.mediacollectioncontents MCC USING (collection_id)
        WHERE MC.username =%s
...
def user_podcast_subscriptions(username):
    ...
    try:
        ...
        FROM mediaserver.subscribed_Podcasts SP JOIN mediaserver.podcast P USING(podcast_id)
        WHERE SP.username =%s
        """
...
def user_in_progress_items(username):
    ...
    try:
        ...
        sql = """
        SELECT MI.media_id,  UMC.play_count AS playcount, UMC.progress, UMC.lastviewed, MI.storage_location
        FROM mediaserver.usermediaconsumption UMC JOIN mediaserver.mediaitem MI USING (media_id)
...
def get_alltvshows():
    ...
    try:
        ...
        FROM mediaserver.tvshow tvs JOIN mediaserver.tvepisode tve USING (tvshow_id)
...
def get_album(album_id):
    ...
    try:
        ...
        # No just id and title if you want album metaData use get_album_metadata()                                         #
        #############################################################################
        sql = """
...
def get_album_songs(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """Select song_id, song_title, artist_name
        From mediaserver.Song JOIN mediaserver.Album_Songs USING (song_id) JOIN mediaserver.Song_Artists USING (song_id) JOIN mediaserver.Artist on (performing_artist_id = artist_id)
        Where album_id = %s
        Order By track_num;
        """
...
def get_album_genres(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """Select distinct album_id, md.md_value as songgenres
...
def get_album_metadata(album_id):
    """
    Get the meta for a album by their ID in your media server
    """
    ...
    try:
        #############################################################################
        # query to get all metadata about a album       #
        #############################################################################

        sql = """Select a.album_id, mdt.md_type_name, md.md_value
        From mediaserver.Album a
        ...
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error getting album metadata for ID: " + album_id, sys.exc_info()[0])
        raise
    cur.close()                     # Close the cursor
    conn.close()                    # Close the connection to the db
    return None
...
def get_tvshow(tvshow_id):
    ...
    try:
        ...
        # including all relevant metadata       #
        #############################################################################
        sql = """Select tvs.tvshow_id, tvs.tvshow_title
...
#####################################################
#   Query (4 b)
#   Get metadata for tvShow
#####################################################
def get_tvshow_metadata(tvshow_id):
    """
    Get the meta for a tvshow by their ID in your media server
    """
    ...
    try:
        #############################################################################
        # query to get all metadata about a tvshow       #
        #############################################################################

        sql = """select tvsmd.tvshow_id, mdt.md_type_name, md.md_value
        ...
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error getting tvshow metadata for ID: "+tvshow_id, sys.exc_info()[0])
        raise
    cur.close()                     # Close the cursor
    conn.close()                    # Close the connection to the db
    return None
