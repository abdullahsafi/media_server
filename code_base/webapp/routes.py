...
def index():
    ...
    # Get a list of user contact details
    user_contacts = None
    user_contacts = database.get_user_contact_details(user_details['username'])
...
def single_podcast(podcast_id):
    ...
    podcast_ep = None
    podcast_ep = database.get_all_podcasteps_for_podcast(podcast_id)
...
def single_podcastep(media_id):
    ...
    # Set up some variables to manage the returns from the database fucntions
    single_podcast = None
    single_podcast = database.get_podcastep(media_id)

    # Once retrieved, do some data integrity checks on the data
    if single_podcast == None:
        single_podcast = []
    # NOTE :: YOU WILL NEED TO MODIFY THIS TO PASS THE APPROPRIATE VARIABLES
...
def single_album(album_id):
    ...
    album_metadata = None
    album_metadata = database.get_album_metadata(album_id)
...
def single_tvshow(tvshow_id):
    ...
    tvshowmetadata = None
    tvshowmetadata = database.get_tvshow_metadata(tvshow_id)
...
def add_song():
    ...
    if request.method == 'POST':
        ...
        if ('song_title' not in request.form):
            newdict['song_title'] = 'Empty title Value'
        else:
        ...
        song = database.add_song_to_db(newdict['storage_location'],newdict['description'],newdict['song_title'],newdict['length'],newdict['song_genre'],newdict['artist_ID'])
